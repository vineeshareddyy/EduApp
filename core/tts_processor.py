...
"""
Unified TTS Processor for Daily Standup + Weekly Interview using Chatterbox.
...
- Stream control (stop/reset) for silence interruption (used by Daily Standup).
...
class UnifiedTTSProcessor:
    ...
    - Async streaming API: generate_ultra_fast_stream(text, session_id=...)
      yields bytes chunks (WAV if soundfile is available, else raw PCM16).
    - Stream control for silence interruption (Daily Standup).
    - health_check() for readiness probes.
    """
    def __init__(
        self,
        ...
        # Stream control for silence interruption (used by Daily Standup)
    ...
    # =========================================================================
    # STREAM CONTROL (used by Daily Standup for silence interruption)
    # =========================================================================
    ...
    def stop_stream(self, session_id: str, stream_type: str = None):
        ...
        logger.info(f"[TTS] Stream stop requested for session {session_id}")

    def should_continue_stream(self, session_id: str) -> bool:
        """Check if streaming should continue."""
        return self._active_streams.get(session_id, True)

    def reset_stream_control(self, session_id: str):
        """Reset stream control to allow new streams."""
        self._active_streams[session_id] = True
