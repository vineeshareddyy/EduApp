...
class UltraFastSessionManagerWithSilenceHandling:
    ...
    async def generate_extended_question(self, session_data) -> str:
        """Generate web-based questions when summary is exhausted."""
        try:
            ...
            max_attempts = 5
            attempted_questions = []
            topic_short = main_topic.split('\n')[0][:100] if main_topic else ""
            
            for attempt in range(max_attempts):
                logger.info(f"🔄 Attempt {attempt + 1}/{max_attempts}")
                
                all_to_avoid = asked_questions + attempted_questions
                
                extended_prompt = prompts.generate_extended_web_question(
                    topic=main_topic,
                    already_asked=all_to_avoid,
                    summary_context=original_summary
                )
                
                if attempt > 0 and attempted_questions:
                    hint = f"\n\n⚠️ REJECTED (same answer as previous):\n"
                    for q in attempted_questions:
                        hint += f"  ❌ {q}\n"
                    hint += "\nAsk something requiring a DIFFERENT answer!"
                    extended_prompt += hint
                ...
                new_question = (new_question or "").strip().strip('"').strip("'").strip()
                
                if not new_question or "?" not in new_question:
                    continue
                
                if len(new_question.split()) < 5:
                    attempted_questions.append(new_question)
                    continue
                
                # ✅ SINGLE LLM CALL to check against ALL questions
                is_duplicate, similar_to = self._is_similar_question(new_question, asked_questions)
                
                if is_duplicate:
                    logger.warning(f"⚠️ Attempt {attempt + 1}: Same answer as previous!")
                    logger.warning(f"   New: '{new_question}'")
                    logger.warning(f"   Same as: '{similar_to}'")
                    attempted_questions.append(new_question)
                    continue
                ...
                session_data.asked_questions.append(new_question)
                
                logger.info(f"✅ Generated unique question (attempt {attempt + 1}): '{new_question}'")
                return new_question
            
            # Fallback
            logger.error(f"❌ Failed after {max_attempts} attempts")
            return f"What else would you like to discuss about {topic_short.split()[0] if topic_short else 'this topic'}?"
    ...
    async def _is_similar_question(self, new_question: str, all_asked_questions: List[str]) -> tuple:
        ...
        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        # QUICK CHECK: Exact match (no LLM needed)
        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        ...
        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        # QUICK CHECK: High word overlap (catch obvious duplicates without LLM)
        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Reply with ONLY: SAME:<number> OR DIFFERENT"""
            ...
            if response.startswith("SAME"):
                try:
                    num = int(response.split(":")[1].strip()) - 1
                    similar_to = questions_to_check[num] if 0 <= num < len(questions_to_check) else questions_to_check[-1]
                except:
                    similar_to = "a previous question"
                
                logger.info(f"🔍 LLM detected same answer!")
                logger.info(f"   New: {new_question}")
                logger.info(f"   Same as: {similar_to}")
                return True, similar_to
            
            logger.info(f"✅ LLM confirmed: Different answer required")
            return False, None
    ...
    async def _force_kill_silence_tts(self, session_id: str, session_data):
        ...
        Does NOT affect: 5-silence termination, counters, or other features.
        """
        logger.info(f"🛑 Force killing silence TTS for session {session_id}")
        
        # Set cancel flag (checked by streaming loop)
        session_data.cancel_silence_tts = True
        session_data.silence_tts_active = False
        
        # Stop TTS stream
        try:
            self.tts_processor.stop_stream(session_id, stream_type="silence")
        except Exception as e:
            logger.warning(f"⚠️ TTS stop_stream failed: {e}")

        # Reset silence prompt state (but NOT the counter - that's for 5-silence logic)
        ...
        try:
            ...
            logger.warning(f"⚠️ Failed to send silence kill signal to frontend: {e}")

    # ============================================================================
    # ✅ NEW: Wrapper to validate ANY question for duplicates
    # ============================================================================
    async def _validate_and_get_unique_question(
        self, 
        session_data: SessionData, 
        question_generator_func, 
        concept_title: str = "unknown",
        max_retries: int = 3
    ) -> str:
        """
        Wrapper that ensures ANY generated question is unique.
        Retries generation if duplicate detected.
        
        Args:
            session_data: Current session
            question_generator_func: Async function that generates a question
            concept_title: Current concept for logging
            max_retries: Number of retry attempts
            
        Returns:
            Unique question string
        """
        asked_questions = getattr(session_data, 'asked_questions', [])
        last_question = None
        ...
        for attempt in range(max_retries):
            # Generate question
            new_question = await question_generator_func()
            new_question = (new_question or "").strip().strip('"').strip("'")
            last_question = new_question
            
            if not new_question or len(new_question) < 10:
                logger.warning(f"⚠️ Generated question too short (attempt {attempt + 1}): '{new_question}'")
                continue
            
            # Check for duplicates using existing method
            is_duplicate, similar_to = self._is_similar_question(new_question, asked_questions)
            
            if is_duplicate:
                logger.warning(f"⚠️ Duplicate detected (attempt {attempt + 1}/{max_retries})")
                logger.warning(f"   New: '{new_question[:60]}...'")
                logger.warning(f"   Similar to: '{similar_to[:60] if similar_to else 'unknown'}...'")
                continue
            
            # SUCCESS - add to tracking and return
            if not hasattr(session_data, 'asked_questions'):
                session_data.asked_questions = []
            session_data.asked_questions.append(new_question)
            
            logger.info(f"✅ Unique question validated (attempt {attempt + 1}): '{new_question[:60]}...'")
            logger.info(f"📊 Total questions tracked: {len(session_data.asked_questions)}")
            return new_question
        
        # Fallback after max retries - return last generated question anyway but log warning
        logger.error(f"❌ Could not generate unique question after {max_retries} attempts for '{concept_title}'")
        logger.error(f"   Using last attempt: '{last_question[:60] if last_question else 'None'}...'")
        
        # Still add to tracking to avoid future duplicates
        if last_question and not hasattr(session_data, 'asked_questions'):
            session_data.asked_questions = []
        if last_question:
            session_data.asked_questions.append(last_question)
        
        return last_question or f"Can you explain more about {concept_title}?"
    ...
    # ============================================================================
    async def _send_silence_response_with_audio(self, session_data, text: str, is_final: bool = False):
        """Send silence response with INTERRUPTION support."""
        session_id = session_data.session_id
        
        try:
            logger.info(f"🤫 Starting silence response: '{text[:80]}' (is_final={is_final})")
            
            # ===== NEW: Mark silence TTS as active =====
            session_data.silence_tts_active = True
            session_data.cancel_silence_tts = False
            self.tts_processor.reset_stream_control(session_id)
            
            # Send text message (YOUR EXISTING CODE)
            await self._send_quick_message(session_data, {
                "type": "silence_response",
                "text": text,
                "status": session_data.current_stage.value if hasattr(session_data, 'current_stage') else "active",
                "is_final": is_final,
                "silence_count": session_data.silence_response_count,
            })
            
            # ===== NEW: Track interruption =====
            interrupted = False
            chunk_count = 0
            ...
            try:
                async for audio_chunk in self.tts_processor.generate_ultra_fast_stream(
                    ...
                    # ===== NEW: INTERRUPT CHECKS =====
                    if getattr(session_data, "cancel_silence_tts", False):
                        logger.info("🛑 Silence TTS interrupted by cancel flag")
                        interrupted = True
                        break
                    
                    if getattr(session_data, "is_user_speaking_live", False):
                        logger.info("🛑 Silence TTS interrupted - user speaking")
                        interrupted = True
                        break
                    
                    # Check recent speech (within 1 second)
                    last_speech = getattr(session_data, "last_user_speech_ts", 0)
                    if last_speech and (time.time() - last_speech) < 1.0:
                        logger.info("🛑 Silence TTS interrupted - recent user speech")
                        interrupted = True
                        break
                    
                    if not session_data.is_active and not is_final:
                        logger.warning(f"⚠️ Session inactive, stopping silence audio")
                        interrupted = True
                        break
                    # ===== END INTERRUPT CHECKS =====
            ...
            # ===== NEW: Only send audio_end if not interrupted =====
            if not interrupted or is_final:
                await self._send_quick_message(session_data, {
                    "type": "audio_end",
                    "status": session_data.current_stage.value if hasattr(session_data, 'current_stage') else "active",
                    "is_final": is_final,
                })
                logger.info(f"✅ Silence response complete: {chunk_count} chunks (is_final={is_final})")
            else:
                logger.info(f"⏹️ Silence response interrupted after {chunk_count} chunks")
            ...
            # ===== NEW: Reset flags on error =====
            session_data.silence_tts_active = False
            session_data.cancel_silence_tts = False
    '''async def _send_silence_response_with_audio(self, session_data: SessionData, text: str, is_final: bool = False):
        """Send silence response using the SAME reliable audio method as regular AI responses."""
        try:
            logger.info(f"🤫 Starting silence response with audio: '{text[:80]}' (is_final={is_final})")
            
            # Send text message with silence_response type AND is_final flag
            await self._send_quick_message(session_data, {
                "type": "silence_response",
                "text": text,
                "status": session_data.current_stage.value,
                "is_final": is_final,  # ✅ NEW: Tell frontend this is the last one
                "silence_count": session_data.silence_response_count,
            })
            
            logger.info(f"✅ Silence response text message sent (is_final={is_final}), generating audio...")
            ...
            await self._send_quick_message(session_data, {
                "type": "audio_end",
                "status": session_data.current_stage.value,
                "is_final": is_final,  # ✅ Also include in audio_end
            })

            if chunk_count == 0:
                logger.error(f"❌ ERROR: ZERO AUDIO CHUNKS generated for silence response!")
            else:
                logger.info(f"✅ Silence response complete: {chunk_count} audio chunks streamed (is_final={is_final})")
    ...
    async def _send_response_with_ultra_fast_audio(self, session_data: SessionData, text: str):
        try:
            session_data.normal_tts_active = True  # <-- ADD
            await self._send_quick_message(session_data, {
                "type": "ai_response",
                "text": text,
                "status": session_data.current_stage.value,
...
def get_grade_from_score(score: float) -> str:
    """Convert numeric score to letter grade (Simplified)."""
